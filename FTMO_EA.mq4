...
void OpenSellOrder()
{
   ...
   int ticket = OrderSend(Symbol(), OP_SELL, LotSize, price, (int)3, sl, tp, 
      ...
      if(OrderSelect(i, SELECT_BY_POS, MODE_TRADES))
      {
         if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber)
         {
            ...
            while(!closed && attempts < maxAttempts)
            {
               ...
               {
                  closed = OrderClose(OrderTicket(), OrderLots(), Ask, (int)3, clrRed);
               }
               ...
               if(!closed)
               {
                  ...
                  Sleep((int)1000); // Wait 1 second before retry
                  attempts++;
               }
               else
               {
